...
double leonardoMFLOPS = 306.31e10;
...
void setup() {
  ...
  display.println("LINPACK_BENCH(64FP)");
  display.printf("Using Leonardo %.1lf PFLOPS", leonardoMFLOPS / 10e9);
  display.display();
  delay(2000);
}
int count = 0;
void loop() {
  ...
  analogWrite(BUILDIN_LED, 255);
  do_benchmark();
  analogWrite(BUILDIN_LED, 0);
  Serial.println("End benchmark");
  ...
  if(potValue >= 1000 && potValue < 2000){
    delay(5000);
  }
  if(potValue >= 2000 && potValue < 3000){
    delay(1000);
  }
  count++;
}
...
# define LDA ( N + 1 )
  ...
  Serial.println ( "\n" );
  Serial.println ( "LINPACK_BENCH\n" );
  Serial.println ( "  C version\n" );
  Serial.println ( "\n" );
  Serial.println ( "  The LINPACK benchmark.\n" );
  Serial.println ( "  Language: C\n" );
  Serial.println ( "  Datatype: Double precision real\n" );
  Serial.print("  Matrix order N               = ");
  Serial.println(N);
  Serial.print("  Leading matrix dimension LDA = ");
  Serial.println(LDA);
...
*/
  ...
  {
    Serial.println ( "\n" );
    Serial.println ( "LINPACK_BENCH - Fatal error!\n" );
    Serial.println ( "  The matrix A is apparently singular.\n" );
    Serial.println ( "  Abnormal end of execution.\n" );
    // OLED
    display.clearDisplay();
    display.setCursor(0, 0);
    display.println("LINPACK_BENCH(64FP)");
    display.println("Fatal error!");
    display.println("The matrix A is apparently singular.");
    display.println("Abnormal end of execution.");
    return 1;
  }
...
*/
  ...
  time[4] = 2.0 / time[3];
  time[5] = total / cray;
  Serial.println ( "     Norm. Resid      Resid           MACHEP         X[1]          X[N]\n" );
  Serial.printf( "  %14f  %14f  %14e  %14f  %14f\n", residn, resid_max, eps, b[0], b[N-1] );
  Serial.println ( "      Factor     Solve      Total     MFLOPS       Unit      Cray-Ratio" );
  Serial.printf ( "  %9f  %9f  %9f  %9f  %9f  %9f\n", 
...
*/
  Serial.println ( "\n" );
  Serial.println ( "LINPACK_BENCH\n" );
  Serial.println ( "Normal execution" );

  // Print to OLED the result
  display.clearDisplay();
  display.setCursor(0, 0);
  display.println("LINPACK_BENCH(64FP)");
  display.printf("Matrix order N: %d\n", N);
  //Results
  display.setCursor(0, 20);
  display.print("Time(s): ");
  display.println(time[2]);
  display.print("MFLOPS: ");
  display.println(time[3]);
  display.print("Cray-Ratio: ");
  display.println(time[5]);
  double compared = leonardoMFLOPS/time[3];
  // Print rounded
  display.printf("Leo Ratio(10e9): %.0lf\n", (compared/10e9));
